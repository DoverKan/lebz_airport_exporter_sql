...
archivo_entrada = "datos_llegadas.txt"
archivo_salida = "consultas_llegadas.sql"
archivo_destinos = "mapeados.txt"  # Archivo con los destinos mapeados

# Configuración de aeropuerto de llegada
aeropuerto_llegada = "Badajoz LEBZ"
...
# Cargar los destinos mapeados desde el archivo
...
for i, linea in enumerate(lineas):
    ...
    if re.match(r"\d{2}:\d{2}", linea):
        try:
            ...
            # Crear consulta SQL para llegada
            consulta = f"""
INSERT INTO `vuelos` (`id`, `tipo`, `company`, `dep`, `arr`, `fecha`, `hora`, `vuelo`, `terminal`) 
VALUES (NULL, 'Llegada', '{aerolinea}', '{destino}', '{aeropuerto_llegada}', '{fecha_formateada}', '{hora}:00', '{vuelo_codigo}', '1');
"""
